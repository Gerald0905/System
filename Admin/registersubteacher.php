...
$max = mysqli_query($conn, "SELECT MAX(teacher_ID) as max FROM tbl_subjectteacher");
$num = mysqli_fetch_assoc($max);

$max1 = mysqli_query($conn, "SELECT MAX(subteacheraccount_ID) as max1 FROM tbl_subteacheraccount");
...
<body>
	...
	<div class="container">
		<div class="header">
			<h4>System Registration for Subject Teachers</h4>
		</div>
			...
			<form method="post">
				...
				<table class="table1">
					...
					<tr>
						...
						<input type="hidden" name="subteacher_ID_hidden" value="<?php echo $num['max'] + 1; ?>">
						<input type="hidden" name="subteacheraccount_ID" value="<?php echo $num1['max1'] + 1; ?>">
					</tr>
					<tr>
					...
					<tr>
						...
						<td  >
                            <select name="subject_teaching" class="chosen" >
				...
				if(isset($_POST['submit'])){
					$subteacher_ID = $_POST['subteacher_ID_hidden'];
					$subteacheraccount_ID = $_POST['subteacheraccount_ID'];
					...
					$insert = mysqli_query($conn, "INSERT INTO tbl_subjectteacher VALUES ('$subteacher_ID', '$teacher_lname','$teacher_fname','$teacher_mname', '$teacher_email_address', '$teacher_sex', '$teacher_bday', '$teacher_address', '$teacher_contact_number', '$strands_name', '$subject_teaching', '$teacher_status')");

					$insert1 = mysqli_query($conn, "INSERT INTO tbl_subteacheraccount VALUES ('$subteacheraccount_ID', '$subteacher_ID', '$teacher_email_address', '$teacher_username', '$teacher_password')");

					if($insert AND $insert1){	
						include('emaill2.php');
						?>
						<script type="text/javascript">
							alert("You have registered  to the system.");
