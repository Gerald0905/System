...
$tid = $_GET['tid'];
$sql = mysqli_query($conn, "SELECT * FROM tbl_teacher as t1 inner join tbl_teacheraccount as t2 on t1.teacher_ID = t2.teacher_ID WHERE t1.teacher_ID = '$tid'");
$rec = mysqli_fetch_assoc($sql);

$sql1 = mysqli_query($conn, "SELECT COUNT(teacheraccount_ID) AS num FROM tbl_teacheraccount");
...
<body>
	...
	<div class="container">
		...
		<form method="post">
			<table>
				<tr>
					<td><input type="hidden" disabled name="teacheraccount_ID" value="<?php echo $IDcount['num']; ?>">
						<input type="hidden" name="teacheraccount_ID_hidden" value="<?php echo $IDcount['num']; ?>">
					</td>
				</tr>
				<tr>
					<td class="head">ID</td>
					<td><input type="number" readonly name="teacher_ID" value="<?php echo $rec['teacher_ID']; ?>"></td>
				</tr>
				<tr>
