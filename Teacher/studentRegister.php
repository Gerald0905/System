...
$tid = $_SESSION['tid'];
$adviserTeacher = mysqli_query($conn, "SELECT * FROM tbl_teacher as t1 inner join tbl_strands as t2 on t1.strands_name = t2.strands_name inner join tbl_tracks as t3 on t2.track_ID = t3.track_ID WHERE t1.teacher_ID = '$tid'");
$adviserResult = mysqli_fetch_assoc($adviserTeacher);
...
<html>
<head>
	<title>Student Registration | Teacher</title>
	<meta charset="utf-8">
			...
			<form method="post">
				...
				<table class="table1">
					...
					<tr>
						...
						<td><input type="text" class="adviser_list" name="middleinitial" placeholder="" value="<?php echo $errorResult['stud_middleinitial']; ?>" required></td>
				...
				<table class="table1">
					...
					<tr>
						...
						<td>
							<select name="track_desc" class="track_list">
								<option selected hidden><p class="desc"><?php echo $adviserResult['track_desc']; ?></p></option>
								<?php
									while($list1 = mysqli_fetch_assoc($track)){
								?>
								<option disabled><?php echo $list1['track_desc']; ?></option>
								<?php } 
								?>
							</select>
						</td>
						...
						<td><select name="strands_name">
								<option selected hidden><p class="desc"><?php echo $adviserResult['strands_name']; ?></p></option>
								<?php
									while($list2 = mysqli_fetch_assoc($strands)){
								?>
								<option disabled><?php echo $list2['strands_name']; ?></option>
								<?php } 
								?>
							</select>
						</td>
					...
					<tr>
						...
						<td><select name="section_name">
								<option selected hidden><p class="desc"><?php echo $adviserResult['section_name']; ?></p></option>
								<?php
									while($list = mysqli_fetch_assoc($sec)){
								?>
								<option disabled><?php echo $list['section_name']; ?></p></option>
								<?php } 
								?>
							</select>
						</td>
